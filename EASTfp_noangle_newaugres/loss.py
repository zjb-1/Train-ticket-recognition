...
def get_geo_loss(gt_geo, pred_geo):
    d1_gt, d2_gt, d3_gt, d4_gt = torch.split(gt_geo, 1, 1)
    d1_pred, d2_pred, d3_pred, d4_pred = torch.split(pred_geo, 1, 1)
...
class Loss(nn.Module):
    ...
    def forward(self, gt_score, pred_score, gt_geo, pred_geo, ignored_map):
        ...
        iou_loss_map = get_geo_loss(gt_geo, pred_geo*(1-ignored_map))

        iou_loss = torch.sum(iou_loss_map*gt_score)/torch.sum(gt_score)
        geo_loss = iou_loss
        print('classify loss is {:.8f}, angle loss is None!!!, iou loss is {:.8f}'.format(classify_loss, iou_loss))
